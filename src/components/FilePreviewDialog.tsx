...
import { Code, Download, Eye, File, FileText, Film, Image as ImageIcon, Loader2, Music, Table, X } from 'lucide-solid';
                    ...
                    <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
                        <div class="flex items-center gap-3 overflow-hidden">
                            <div class="p-2 bg-primary/10 rounded-lg text-primary shrink-0">
                                <Switch fallback={<File size={20} />}>
                                    <Match when={getFileType(props.fileKey) === 'image'}>
                                        <ImageIcon size={20} />
                                    </Match>
                                    <Match when={getFileType(props.fileKey) === 'video'}>
                                        <Film size={20} />
                                    </Match>
                                    <Match when={getFileType(props.fileKey) === 'audio'}>
                                        <Music size={20} />
                                    </Match>
                                    <Match when={getFileType(props.fileKey) === 'text'}>
                                        <FileText size={20} />
                                    </Match>
                                </Switch>
                            </div>
                        ...
                        <Show when={!loading() && !error() && contentUrl()}>
                            <div class="w-full h-full overflow-auto flex items-center justify-center p-4">
                                <Switch fallback={
                                    ...
                                    <Match when={getFileType(props.fileKey) === 'image' || getFileType(props.fileKey) === 'svg'}>
                                        <img src={contentUrl()!} alt={props.fileKey} class="max-w-full max-h-full object-contain shadow-lg rounded-lg bg-[url('/checkerboard.svg')] bg-white" />
                                    </Match>
                                    <Match when={getFileType(props.fileKey) === 'video'}>
                                        <video src={contentUrl()!} controls class="max-w-full max-h-full shadow-lg rounded-lg bg-black" />
                                    </Match>
                                    <Match when={getFileType(props.fileKey) === 'audio'}>
                                    ...
                                    <Match when={getFileType(props.fileKey) === 'pdf'}>
                                        <iframe src={contentUrl()!} class="w-full h-full shadow-lg bg-white rounded-lg" />
                                    </Match>
                                    <Match when={getFileType(props.fileKey) === 'text'}>
                                                ...
                                                <Show when={extension() === 'csv' && viewMode() === 'table' && csvContent()} fallback={
                                                    ...
                                                    <div class="w-full h-full overflow-auto">
                                                        <table class="w-full border-collapse text-sm text-left">
                                                            <thead>
                                                                <tr>
                                                                    <For each={csvContent()![0]}>{(header: string) => (
                                                            ...
                                                            <tbody>
                                                                <For each={csvContent()!.slice(1)}>{(row: string[]) => (
                                                                    <tr class="border-b border-slate-100 hover:bg-slate-50/50 transition-colors">
                                                                        <For each={row}>{(cell: string) => (
